...
if ($associationId <= 0 || !in_array($action, ['approve', 'reject'], true)) {
...
try {
    ...
    $stmt = $pdo->prepare(
        "UPDATE associations
         SET status = :status, verified = :verified, verification_date = NOW(), updated_at = NOW()
         WHERE id = :id"
    );
    $stmt->execute([':status' => $newStatus, ':verified' => $verified, ':id' => $associationId]);
    ...
    json_response([
        'success'    => true,
        'message'    => $action === 'approve' ? 'Association approuvée.' : 'Association rejetée.',
