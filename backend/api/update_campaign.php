...
try {
    ...
    $campaignId = (int) ($_POST['campaign_id'] ?? ($_POST['request_id'] ?? 0));
    ...
    if ($campaignId <= 0 || $title === '' || $description === '' || $location === '' || $categoryId <= 0 || $goalAmount <= 0) {
    ...
    $updateQuery = 'UPDATE campaigns SET
        title = :title,
